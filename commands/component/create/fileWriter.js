...
export const createComponentFiles = async ({ name, scope, state, redux, storybook }) => {
	...
	fs.mkdir(componentFolder, () => {
		...
		const containerContent = genContainerContent(componentName, state, redux);
		...
		if (state || redux) {
			indexWriteStream.write(containerContent);
		} else {
		...
		// create component.js
		if (state || redux) {
			const componentFile = path.join(componentFolder, 'component.js');
			const componentStream = fs.createWriteStream(componentFile);
			componentStream.write(componentContent);
			componentStream.end();

			componentStream.on('finish', () => {
				console.log('component.js 建立完成');
			});
		}
